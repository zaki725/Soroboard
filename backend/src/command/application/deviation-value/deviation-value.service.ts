...
export class DeviationValueService {
  ...
  async create(params: CreateParams): Promise<DeviationValueResponseDto> {
    ...
    const createEntity = new CreateDeviationValueEntity({
  ...
  async update(params: UpdateParams): Promise<DeviationValueResponseDto> {
    ...
    const updateEntity = new UpdateDeviationValueEntity({
      id: params.id,
      value: params.value,
      updatedBy: params.userId,
    });

    const updated = await this.deviationValueRepository.update(updateEntity);
