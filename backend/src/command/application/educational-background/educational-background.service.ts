...
import type { EducationType } from '../../domain/educational-background/educational-background.types';
...
export class EducationalBackgroundService {
  ...
  async create(
    ...
    const createEntity = EducationalBackgroundEntity.createNew({
  ...
  async update(
    ...
    const existing = await this.educationalBackgroundRepository.findOne({
      id: params.id,
    });

    if (!existing) {
      throw new NotFoundError('学歴', params.id);
    }

    existing.updateEducationInfo({
      educationType: params.educationType,
      universityId: params.universityId,
      facultyId: params.facultyId,
      graduationYear: params.graduationYear,
    ...
    await this.educationalBackgroundRepository.update(existing);
